...
// USER_ROLES env var format: "id1:admin,id2:user"
function getRoleForAccessId(env, accessId) {
    ...
    for (const entry of envList(env, "USER_ROLES")) {
          ...
          const role = entry.slice(colonIndex + 1).trim().toLowerCase();
          if (constantTimeEqual(id, normalized)) return role;
    }
    return "user";
}
...
async function setSession(request, env, accessId) {
    ...
    const role = getRoleForAccessId(env, accessId);
    const session = {
...
async function routeApi(request, env) {
  ...
  if (url.pathname === "/api/realtime-token" && request.method === "POST") {
        const session = await getSession(request, env);
        if (!session) return json({ error: "Not authenticated" }, 401);
        if ((session.role || "user") === "admin" || (session.role || "user") === "user") {
  ...
  if (url.pathname === "/api/admin/users" && request.method === "GET") {
      ...
      const allowedIds = envList(env, "ALLOWED_LOGIN_IDS").map((id) => ({
              id,
              role: getRoleForAccessId(env, id)
      }));

      return json({ users: allowedIds });
  }
